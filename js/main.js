...
$(".title").on("click", function (e) {
  $(e.target).next().animate({ with: "toggle" }, 100);
  $(".para").not($(e.target).next()).slideUp();
});

$("textarea").on("input", function (e) {
  let hamada = e.target.value.length;
 $('.num').text(
  100 - hamada 
 )
});
...
$('a').on('click',function(e){
  ...
  $('body').animate({scrollTop:sectionOffest }, 60000)
})
